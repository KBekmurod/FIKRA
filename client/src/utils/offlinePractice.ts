import type { Question, ExamSession as ExamSessionType, SubjectBreakdown } from '../types'

export type OfflineQuestion = Question & {
  answer: number
  explanation?: string
}

export interface ExamConfigSubject {
  id: string
  name: string
...
export interface ExamConfigDirection {
  id: string
  name: string
  spec1: string
  spec2: string
...
export interface ExamConfigCache {
  directions: ExamConfigDirection[]
...
type BankMap = Record<string, OfflineQuestion[]>
...
function getBank(): BankMap {
  return safeParse<BankMap>(localStorage.getItem(BANK_KEY), {})
}

function setBank(bank: BankMap) {
  localStorage.setItem(BANK_KEY, JSON.stringify(bank))
}

export function getCachedExamConfig(): ExamConfigCache | null {
  return safeParse<ExamConfigCache | null>(localStorage.getItem(CONFIG_KEY), null)
}

export function saveCachedExamConfig(config: ExamConfigCache) {
  localStorage.setItem(CONFIG_KEY, JSON.stringify(config))
}

export function saveOfflinePack(subject: string, block: string, questions: OfflineQuestion[]) {
  const bank = getBank()
...
function pickQuestions(subject: string, block: string, count: number): OfflineQuestion[] {
  const bank = getBank()
  const key = `${subject}__${block}`
  const list = [...(bank[key] || [])]
...
function buildSubjectBreakdown(entries: Array<{ id: string; name: string; block: string; weight: number; count: number }>): SubjectBreakdown[] {
...
function buildDuration(totalQuestions: number, mode: 'dtm' | 'subject') {
  if (mode === 'dtm') return 180 * 60
  return Math.max(15 * 60, totalQuestions * 90)
}

export async function warmOfflineQuestionBank(
  config: ExamConfigCache,
  fetchPack: (subject: string, block: string, limit: number) => Promise<OfflineQuestion[]>,
) {
...
export function buildOfflineDtmSession(config: ExamConfigCache, directionId: string): OfflineSessionData | null {
...
export function buildOfflineSubjectSession(
  config: ExamConfigCache,
  ...
  const entries = selectedSubjects
    ...
    .filter(Boolean) as Array<{ id: string; name: string; block: string; weight: number; count: number }>
...
export function calculateOfflineResult(session: OfflineSessionData, answers: Record<string, { selected: number; isCorrect: boolean }>) {
