...
export default function TestResultPage() {
    ...
    var _c = useState(false), showConfetti = _c[0], setShowConfetti = _c[1];
    var _d = useState(null), miniTestData = _d[0], setMiniTestData = _d[1];
    useEffect(function () {
        if (state) {
            ...
            if (state.miniTestId) {
                api.get("/api/history/exams").then(function (_a) {
                    var data = _a.data;
                    var mini = data.sessions ? .find(function (s) { return String(s._id) === String(state.miniTestId); })
                        :
                    ;
            ...
            examApi.review(sessionId).then(function (_a) {
                var data = _a.data;
                ...
                if (s.miniTestId) {
                    api.get("/api/history/exams").then(function (_a) {
                        var hist = _a.data;
                        var mini = hist.sessions ? .find(function (ss) { return String(ss._id) === String(s.miniTestId); })
                            :
                        ;
                        if (mini && mini.status === 'completed') {
                            setMiniTestData(mini);
    ...
    if (!state || !sessionId) {
        ...
        <button onClick={function () { return navigate('/testlar'); }} className="btn btn-primary">
          Testlarga qaytish
        </button>
      </div>);
    }
    var totalScore = state.totalScore, maxTotalScore = state.maxTotalScore, percent = state.percent, level = state.level, mode = state.mode, isMini = state.isMini;
    var grade = percent >= 90 ? 'A\'lo' : percent >= 75 ? 'Yaxshi' : percent >= 50 ? "O'rtacha" : 'Yaxshilash kerak';
    var emoji = percent >= 80 ? '🏆' : percent >= 60 ? '👏' : percent >= 40 ? '💪' : '📖';
    var hasErrors = state.subjectBreakdown.some(function (b) { return b.wrong > 0; });
    var totalCorrect = state.subjectBreakdown.reduce(function (sum, b) { return sum + (b.correct || 0); }, 0);
    var totalQuestions = state.subjectBreakdown.reduce(function (sum, b) { return sum + (b.questionCount || 0); }, 0);
    var cardBtn = function (active) { return ({
    ...
    return (<>
      ...
      <div style={{ padding: '8px 20px 0' }}>
        ...
        overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'
            ...
            {' · '}
            {state.subjectBreakdown.map(function (b) { return b.subjectName || b.subjectId; }).join(', ')}
          </div>
        </div>
        ...
        {state.subjectBreakdown.length > 0 && (<div style={{ marginTop: 14 }}>
        ...
        display: 'grid', gap: 8
              ...
              {state.subjectBreakdown.map(function (b) {
        var subj = SUBJECTS[b.subjectId];
        var bPct = b.questionCount > 0 ? Math.round((b.correct / b.questionCount) * 100) : 0;
        ...
        {miniTestData && !isMini && (<div style={{ marginTop: 14 }}>
            ...
            <button onClick={function () { return navigate("/test-result/" + miniTestData._id); }} style={{
        width: '100%',
      ...
      <div style={{ padding: '0 20px', display: 'grid', gap: 10 }}>
        ...
        <button onClick={function () { return navigate("/test-review/" + sessionId, { state: state }); }} style={cardBtn(true)}>
          <div style={{ fontSize: 32 }}>📊</div>
          <div style={{ flex: 1 }}>
        ...
        <button onClick={function () { return navigate("/test-explain/" + sessionId + "/_overview", { state: state }); }} disabled={!hasErrors} style={__assign({}, cardBtn(hasErrors), { background: hasErrors ? 'linear-gradient(135deg, rgba(123,104,238,0.12), rgba(167,139,250,0.05))' : 'var(--s2)', border: "1.5px solid " + (hasErrors ? 'rgba(123,104,238,0.3)' : 'var(--f)') })}>
          <div style={{ fontSize: 32 }}>🎯</div>
          <div style={{ flex: 1 }}>
        ...
        <button onClick={function () { return navigate('/tarix'); }} style={cardBtn(true)}>
          <div style={{ fontSize: 32 }}>📚</div>
          <div style={{ flex: 1 }}>
      ...
      <div style={{ padding: '24px 20px' }}>
        <button onClick={function () { return navigate('/testlar'); }} className="btn btn-ghost btn-block">Testlar sahifasiga qaytish</button>
      </div>
    </>);
}
