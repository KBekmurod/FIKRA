...
export default function TestReviewPage() {
    ...
    var _a = useState(initial || null), data = _a[0], setData = _a[1];
    ...
    useEffect(function () {
        ...
        examApi.review(sessionId)
            .then(function (_a) {
            var data = _a.data;
            setData(function (prev) { return (__assign({}, prev, (data.session || data))); });
            setAnswers(data.answers || []);
        })["catch"](function () { return toast.error("Ma'lumotlar yuklanmadi"); })["finally"](function () { return setLoading(false); });
    }, [sessionId]);
    if (loading || !data) {
    ...
    return (<>
      ...
      <div style={{ padding: '8px 20px 0' }}>
        ...
        var acc = b.questionCount > 0 ? Math.round((b.correct / b.questionCount) * 100) : 0;
      ...
      {answers.length > 0 && (<>
          ...
          <div style={{ padding: '0 20px 12px', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <button onClick={function () { setSubjectFilter('all'); setPage(1); }} style={{
        ...
        cursor: 'pointer'
          ...
          <div style={{ padding: '0 20px', display: 'grid', gap: 8 }}>
            {pageItems.map(function (a, i) { return (<AnswerCard key={a._id || i} answer={a} index={(page - 1) * PAGE_SIZE + i}/>); })}
          </div>

          
          {totalPages > 1 && (<div style={{ padding: '16px 20px', display: 'flex', justifyContent: 'center', gap: 8 }}>
              <button disabled={page === 1} onClick={function () { return setPage(function (p) { return p - 1; }); }} className="btn btn-ghost" style={{ opacity: page === 1 ? 0.4 : 1 }}>← Oldingi</button>
              <span style={{
        padding: '8px 14px',
        fontSize: 12,
        fontWeight: 700,
        color: 'var(--txt-2)'
              ...
              <button disabled={page === totalPages} onClick={function () { return setPage(function (p) { return p + 1; }); }} className="btn btn-ghost" style={{ opacity: page === totalPages ? 0.4 : 1 }}>Keyingi →</button>
            </div>)}
        </>)}
...
function AnswerCard(_a) {
    var answer = _a.answer, index = _a.index;
    var _b = useState(false), open = _b[0], setOpen = _b[1];
    var correct = answer.isCorrect;
    var skipped = answer.selectedOption === null || answer.selectedOption === undefined;
    return (<div style={{
        background: 'var(--s1)',
      ...
      <button onClick={function () { return setOpen(function (o) { return !o; }); }} style={{
      ...
      {open && (<div style={{ padding: '0 14px 12px' }}>
          ...
          {(answer.questionOptions || answer.options || []).map(function (opt, i) {
        var color = 'var(--txt-3)', icon = '';
        var correctIdx = answer.correctAnswer !== undefined ? answer.correctAnswer : answer.correctIndex;
        if (i === correctIdx) {
            color = 'var(--g)';
            icon = '✓ ';
        }
