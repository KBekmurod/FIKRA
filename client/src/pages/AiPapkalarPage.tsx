          ...
          <div style={{ fontSize: 10.5, color: 'var(--txt-3)', marginTop: 2 }}>
            {!hasFolders
              ...
              : <span>
                  ...
                  {stats!.avgScore > 0 && (
                    <> · <strong style={{ color: stats!.avgScore >= 70 ? 'var(--g)' : stats!.avgScore >= 50 ? 'var(--y)' : 'var(--r)' }}>{stats!.avgScore}%</strong></>
                  )}
                </span>
            }
          </div>
    ...
    <>
      ...
      <div style={{ padding: '6px 20px 0' }}>
        ...
        <div className="seg-tabs">
          <button className={`seg-tab ${tab === 'majburiy' ? 'active' : ''}`}
            onClick={() => setTab('majburiy')}>Majburiy</button>
          <button className={`seg-tab ${tab === 'mutaxassislik' ? 'active' : ''}`}
            onClick={() => setTab('mutaxassislik')}>Mutaxassislik</button>
        </div>
