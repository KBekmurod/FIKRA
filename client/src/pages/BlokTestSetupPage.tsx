          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {SPEC_SUBJECTS.map(s => {
                ...
                const activeIndex = customSubjects.indexOf(s.id)
                return (
                  <button
                    key={s.id}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
            <span>
              {mode === 'custom' && customSubjects.length > 0 
                ? `1-mutaxassislik (${SPEC_SUBJECTS.find(x => x.id === customSubjects[0])?.name})` 
                : mode === 'block' && selectedBlock 
                  ? `1-mutaxassislik (${DIRECTION_BLOCKS.find(x => x.id === selectedBlock)?.subjects[0].name})`
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <span>
              {mode === 'custom' && customSubjects.length > 1 
                ? `2-mutaxassislik (${SPEC_SUBJECTS.find(x => x.id === customSubjects[1])?.name})` 
                : mode === 'block' && selectedBlock 
                  ? `2-mutaxassislik (${DIRECTION_BLOCKS.find(x => x.id === selectedBlock)?.subjects[1].name})`
