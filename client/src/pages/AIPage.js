    ...
    return (<div className="chat-wrap">
      ...
      {!canChat ? (<div className="chat-input-bar">
        ...
        if (e.key === 'Enter' && !e.shiftKey) {
          ...
          {sending ? (<button onClick={abortChat} className="btn" style={{ width: 44, height: 44, padding: 0, flexShrink: 0, background: 'var(--r)', color: 'white', border: 'none', borderRadius: '12px' }} title="To'xtatish">
              ⏹
            </button>) : (<button disabled={!input.trim()} onClick={send} className="btn btn-primary" style={{ width: 44, height: 44, padding: 0, flexShrink: 0 }}>
              →
            </button>)}
        </div>)}
    </div>);
